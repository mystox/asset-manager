...
!function (a) {
	...
	a.laydate = function (b) {
		...
		a.style.left = e.left + (b ? 0 : c.scroll(1)) + 'px', d = e.bottom + a.offsetHeight / 1.5 <= c.winarea() ? e.bottom - 1 : e.top > a.offsetHeight / 1.5 ? e.top - a.offsetHeight + 1 : c.winarea() - a.offsetHeight, a.style.top = d + (b ? 0 : c.scroll()) + 3 + 'px';
	}, c.follow = function (a) {
		...
		c.addClass(d.body, 'laydate_body'), h.tds = b('#laydate_table td'), h.mms = b('#laydate_ms span'), h.year = b('#laydate_y'), h.month = b('#laydate_m'), c.each(b(e.box + ' .laydate_ym'), function (a, b) {
			...
			c.elem[h.elemv] = '';
			typeof c.options.choose === 'function' && c.options.choose('');
			c.close();
		}), h.otoday = b('#laydate_today'), c.on(h.otoday, 'click', function () {
			c.elem[h.elemv] = laydate.now(0, c.options.format);
			typeof c.options.choose === 'function' && c.options.choose(laydate.now(0, c.options.format));
			c.close();
